...
import { ArrowLeft, Plus, Trash2, ImageOff, Check } from 'lucide-react';
import { toast } from 'sonner';
import { useQuery } from '@tanstack/react-query';
import { api } from '@/lib/api';
import { useAuthStore } from '@/lib/store';
import { useUpdateProduct } from '@/hooks/use-shop-management';
import {
  Card,
  CardHeader,
  CardTitle,
...
export default function EditProductPage({
  ...
  const { data: product, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get<Product>('/products/' + id, accessToken ?? undefined);
  ...
  useEffect(() => {
    ...
    setIsUnlimited(product.stock?.isUnlimited ?? false);
    setStockQuantity(String(product.stock?.quantity ?? 0));
    setImages(
    ...
    setFlowers(
      product.flowerComposition?.map((f) => ({
        flowerId: f.flowerId,
    ...
    setEmotions(product.emotions ?? []);
    setCustomOptions(
      product.customizationOptions?.map((opt) => ({
        name: opt.name,
        type: opt.type,
        options: opt.options?.join(', ') ?? '',
        priceModifier: String(opt.priceModifier ?? 0),
      })) ?? [],
    );
  }, [product]);
  ...
  const updateFlower = (idx: number, field: keyof FlowerEntry, value: string) => {
    setFlowers((prev) => prev.map((f, i) => (i === idx ? { ...f, [field]: value } : f)));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      stock: { quantity: isUnlimited ? 0 : Number(stockQuantity), isUnlimited },
      images: validImages,
      flowerComposition:
        flowers.length > 0
          ? flowers
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <span className="text-sm font-semibold text-gray-900">Chỉnh sửa sản phẩm</span>
      </div>
      ...
      <Section title="Giá & Kho hàng">
        ...
        <div className="space-y-3">
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            <span className="text-sm font-medium text-gray-700">Không giới hạn số lượng</span>
          </label>
          {!isUnlimited && (
            <Input
      ...
      <Section title="Dịp & Cảm xúc">
        <div>
          ...
          <TagToggle items={OCCASIONS} selected={occasions} onChange={setOccasions} />
        </div>
        <div>
          <p className="mb-2 text-sm font-medium text-gray-700">Cảm xúc muốn truyền tải</p>
          <TagToggle items={EMOTIONS} selected={emotions} onChange={setEmotions} />
        </div>
      </Section>
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <Input
                    ...
                    onChange={(e) => updateCustomOption(idx, 'priceModifier', e.target.value)}
                    min="0"
                    step="1000"
                  />
                </div>
